defmodule ExAcl.Routes do
  ...
  def filter_method_and_path_array(path_list) do
    Enum.reduce(path_list, [], fn path, acc ->
      case path do
        [_phx_controller_path, method, path_url, _controller_name, _controller_func_name]
        when method in ["GET", "POST", "DELETE", "PATCH", "PUT"] ->
          acc ++ [{method, path_array(path_url)}]

        [method, path_url, _controller_name, _controller_func_name]
        when method in ["GET", "POST", "DELETE", "PATCH", "PUT"] ->
          acc ++ [{method, path_array(path_url)}]
  ...
  defp path_array(path) do
    path |> String.split("/") |> Enum.filter(fn s -> s != "" end)
  end
